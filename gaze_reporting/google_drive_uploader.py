...
class GoogleDriveUploader:
    ...
    def get_or_create_root_folder(self) -> Optional[str]:
        ...
        print(f"📁 Looking for '{DRIVE_FOLDER_NAME}' folder...")
        ...
        try:
            ...
            if folders:
                ...
                # Create new folder
                print(f"📁 Creating new folder: {DRIVE_FOLDER_NAME}")
                folder_metadata = {
                ...
                print(f"✅ Created folder: {DRIVE_FOLDER_NAME}")
                return folder_id
    ...
    def get_or_create_date_folder(self, date_str: str) -> Optional[str]:
        ...
        print(f"📅 Looking for date folder: {date_str}")
        ...
        try:
            ...
            if folders:
                folder_id = folders[0]['id']
                print(f"✅ Found existing date folder: {date_str}")
                return folder_id
            else:
                # Create new date folder
                print(f"📁 Creating date folder: {date_str}")
                folder_metadata = {
                    'name': date_str,
                    'mimeType': 'application/vnd.google-apps.folder',
                ...
                folder_id = folder.get('id')
                print(f"✅ Created date folder: {date_str}")
                return folder_id
    ...
    def compress_session_folder(self, session_path: str) -> Optional[str]:
        ...
        if not session_path.is_dir():
            print(f"❌ Path is not a directory: {session_path}")
            return None
        ...
        print(f"📦 Compressing {session_path.name}...")
        ...
        try:
            ...
            print(f"✅ Compressed to {zip_filename} ({zip_size / (1024*1024):.1f} MB)")
            return str(zip_path)
    ...
    def get_shareable_link(self, file_id: str) -> str:
        ...
        try:
            ...
            print(f"⚠️ Could not create shareable link: {e}")
            return ""
    ...
    def upload_session(self, session_path: str, cleanup: bool = True) -> bool:
        ...
        if cleanup:
            try:
                os.remove(zip_path)
                print(f"🗑️ Cleaned up local file: {zip_filename}")
            except Exception as e:
        ...
        print("="*60)
        print(f"📁 Folder: {DRIVE_FOLDER_NAME}/{date_str}")
